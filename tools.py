...
class GetFromGui(wx.Dialog):
    ...
    def __init__(self, parent, id, title):
        wx.Dialog.__init__(self, parent, id, title, size=(280,375))
        # Add text labels
        ...
        wx.StaticText(self, -1, 'Orientation:', pos=(40,55))
        self.sc_ori = wx.SpinCtrl(self, -1, '', (155,50))
        self.sc_ori.SetRange(0,180)
        self.sc_ori.SetValue(0)

        self.rb_task1 = wx.RadioButton(self, -1, 'Left', (55, 100),
                                  style=wx.RB_GROUP)
        self.rb_task2 = wx.RadioButton(self, -1, 'Right', (143, 100))
        self.rb_task1.SetValue(1)

        self.rb_fix = wx.RadioButton(self, -1, 'fixation task', (100, 150),
                                  style=wx.RB_GROUP)

        wx.StaticText(self, -1, 'Start fixation task:', pos=(40,170))
        self.text_fix = wx.TextCtrl(self, -1, pos=(155,170), size=(40, -1))

        self.rb_per = wx.RadioButton(self, -1, 'peripheral task', (100, 205),
                                  style=wx.RB_GROUP)
        wx.StaticText(self, -1, 'Start peripheral task:', pos=(20,230))
        self.text_per = wx.TextCtrl(self, -1, pos=(155,230), size=(40, -1))


        self.rb_demo = wx.RadioButton(self, -1, 'demo', (100, 270),
                                  style=wx.RB_GROUP)
        # Add OK/Cancel buttons
        wx.Button(self, 1, 'Done', (60, 320))
        wx.Button(self, 2, 'Quit', (150, 320))
    ...
    def OnDone(self,event):
        ...
        if self.rb_fix.GetValue():
            self.do_fix = True
        else:
            self.do_fix = False

        if self.rb_per.GetValue():
            self.do_per = True
        else:
            self.do_per = False


        self.start_per = self.text_per.GetValue()
        if self.start_per == '':
            self.start_per='0.25'

        self.start_fix = self.text_fix.GetValue()
        if self.start_fix == '':
            self.start_fix='0.25'
        self.sc_ori = self.sc_ori.GetValue()
        self.Close()
...
def start_data_file(subject_id):
    ...
    i=1
    this_data_file = '%s_%s_%s_fiorentini.csv'%(subject_id,
...
class Text(object):
    ...
    def __call__(self,duration=np.inf,key_press=True):
        """
        Text is shown to the screen, until a key is pressed (if key_press is True) or until duration
        elapses (default = inf)
